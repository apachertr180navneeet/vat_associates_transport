...
<div class="container-fluid flex-grow-1 container-p-y">
    <div class="row">
        <div class="col-md-6 text-start">
            <h5 class="py-2 mb-2">
                <span class="text-primary fw-light">Department type</span>
            </h5>
        </div>
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="exampleModalLabel1">Department type Add</h5>
...
<div class="modal fade" id="editModal" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="exampleModalLabel1">Department type Edit</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body">
                <div class="row">
...
<script>
    $(document).ready(function() {
        const table = $('#locationTable').DataTable({
            processing: true,
            ajax: {
                url: "{{ route('admin.departmenttype.getall') }}",
                type: 'GET',
            },
            columns: [
                { data: "name" },
        ...
        $('#AddLocation').click(function(e) {
            ...
            $.ajax({
                url: '{{ route('admin.departmenttype.store') }}',
                type: 'POST',
                data: data,
        ...
        window.editLocation = function(userId) {
            const url = '{{ route("admin.departmenttype.get", ":userid") }}'.replace(":userid", userId);
            $.ajax({
                url: url,
                method: 'GET',
        ...
        $('#EditLocation').on('click', function() {
            ...
            $.ajax({
                url: '{{ route('admin.departmenttype.update') }}',
                method: 'POST',
                data: {
                    _token: $('meta[name="csrf-token"]').attr('content'),
        ...
        window.updateUserStatus = function(userId, status) {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        type: "POST",
                        url: "{{ route('admin.departmenttype.status') }}",
        ...
        window.deleteUser = function(userId) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    const url = '{{ route("admin.departmenttype.destroy", ":userId") }}'.replace(":userId", userId);
                    $.ajax({
                        type: "DELETE",
                        url,
                        ...
                        success: function (response) {
                            if (response.success) {
                                ...
                                setFlash("error", "There was an issue deleting the departmenttype. Please contact your system administrator.");
                            }
                            table.ajax.reload();
                        },
                        error: function () {
