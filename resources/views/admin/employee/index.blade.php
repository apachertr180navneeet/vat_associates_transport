        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <div class="row">
                    ...
                    <div class="col-md-12 mb-3">
                        <label for="breanch" class="form-label">Branch</label>
                        <select id="breanch" class="form-select form-select">
                    ...
                    <div class="col-md-12 mb-3">
                        <label for="deaprtment" class="form-label">Department</label>
                        <select id="deaprtment" class="form-select form-select">
                            <option value="">select</option>
                    ...
                    <div class="col-md-12 mb-3">
                        ...
                        <input type="date" id="birthday" class="form-control" placeholder="Enter Birthday" />
                        <small class="error-text text-danger"></small>
                    </div>
                    <div class="col-md-12 mb-3">
                        <label for="anniversary" class="form-label">Anniversary</label>
                        <input type="date" id="anniversary" class="form-control" placeholder="Enter Name" />
                        <small class="error-text text-danger"></small>
                    </div>
                </div>
            </div>
...
<div class="modal fade" id="editModal" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="exampleModalLabel1">Employee Edit</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            ...
            <div class="modal-body">
                <div class="row">
                    ...
                    <div class="col-md-12 mb-3">
                        <label for="editbreanch" class="form-label">Branch</label>
                        <select id="editbreanch" class="form-select form-select">
                            <option value="">select</option>
                            @foreach ( $branchs as $branch )
                                <option value="{{ $branch->id }}">{{ $branch->name }}</option>
                            @endforeach
                    ...
                    <div class="col-md-12 mb-3">
                        <label for="editdeaprtment" class="form-label">Department</label>
                        <select id="editdeaprtment" class="form-select form-select">
                            <option value="">select</option>
                            @foreach ( $departments as $department )
                                <option value="{{ $department->id }}">{{ $department->name }}</option>
                            @endforeach
                    ...
                    <div class="col-md-12 mb-3">
                        ...
                        <input type="date" id="editbirthday" class="form-control" placeholder="Enter Birthday" />
                        <small class="error-text text-danger"></small>
                    </div>
                    <div class="col-md-12 mb-3">
                        <label for="editanniversary" class="form-label">Anniversary</label>
                        <input type="date" id="editanniversary" class="form-control" placeholder="Enter Name" />
                        <small class="error-text text-danger"></small>
                    </div>
                </div>
            </div>
...
<script>
    $(document).ready(function () {
        ...
        $('#AddItem').click(function(e) {
            ...
            $.ajax({
                url: '{{ route('admin.employee.store') }}', // Adjust the route as necessary
                type: 'POST',
                data: data,
        ...
        function editUser(userId) {
            const url = '{{ route("admin.employee.get", ":userid") }}'.replace(":userid", userId);
            $.ajax({
                ...
                success: function(data) {
                    ...
                    $('#editbreanch').val(data.deaprtment);
                    $('#editdeaprtment').val(data.breanch);
                    $('#editbirthday').val(data.birthday);
                    $('#editanniversary').val(data.anniversary);
        ...
        $('#EditComapany').on('click', function() {
            ...
            $.ajax({
                url: '{{ route('admin.employee.update') }}', // Update this URL to match your route
                method: 'POST',
                data: {
                    ...
                    deaprtment: $('#editbreanch').val(),
                    breanch: $('#editdeaprtment').val(),
        ...
        function deleteUser(userId) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    const url = '{{ route("admin.employee.destroy", ":userId") }}'.replace(":userId", userId);
                    $.ajax({
                        type: "DELETE",
                        url,
