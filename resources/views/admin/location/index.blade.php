    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="exampleModalLabel1">Location Add</h5>
...
<div class="modal fade" id="editModal" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="exampleModalLabel1">Location Edit</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body">
                <div class="row">
...
<script>
    $(document).ready(function() {
        ...
        window.editLocation = function(userId) {
            const url = '{{ route("admin.location.get", ":userid") }}'.replace(":userid", userId);
            $.ajax({
                url: url,
                method: 'GET',
        ...
        window.updateUserStatus = function(userId, status) {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        type: "POST",
                        url: "{{ route('admin.location.status') }}",
        ...
        window.deleteUser = function(userId) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    const url = '{{ route("admin.location.destroy", ":userId") }}'.replace(":userId", userId);
                    $.ajax({
                        type: "DELETE",
                        url,
                        ...
                        success: function (response) {
                            if (response.success) {
                                ...
                                setFlash("error", "There was an issue deleting the location. Please contact your system administrator.");
                            }
                            table.ajax.reload();
                        },
                        error: function () {
